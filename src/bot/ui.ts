...
export const menuKeyboard = (verified: boolean) => {
  ...
  if (verified) {
    rows.push([{ text: '?? Create act from submission', payload: CB.MENU_IMPORT }]);
  }
  rows.push([{ text: '?? Enter manually', payload: CB.MENU_MANUAL }]);
  rows.push([{ text: '?? Top up balance', payload: CB.MENU_TOPUP }]);
  rows.push([{ text: verified ? '??? Act history' : '??? History', payload: CB.MENU_HISTORY }]);
  rows.push([{ text: '? Help', payload: CB.MENU_HELP }]);
